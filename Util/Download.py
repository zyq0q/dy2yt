...
class Download:
    ...
    async def AwemeDownload(self, aweme_data):
        ...
        print('++++++++++++++++++++++++++++++++++++++++++++++++')
        sorted_data = sorted(aweme_data, key=lambda x: x['create_time'], reverse=True)
        ...
        try: 
            ...
            dy_js = []
            up_data = {}
            aweme_id = first_element['aweme_id']
            if (os.path.exists(dy_file)):
                dy_js = json.load(open(dy_file))
                for item in dy_js:
                    dy_vedio_id_list.append(item['aweme_id'])
            if (aweme_id in dy_vedio_id_list):
                print('视频已下载.......')
                return
            else:
        ...
        for aweme in new_set:
            ...
            up_data['title'] = first_element['desc']
            up_data['description'] = first_element['desc']
            up_data['keywords'] = '搞笑,日常,日常,翻车，专治不开心'
            up_data['category'] = '23'
            up_data['privacyStatus'] = 'public'
            # up_data['url'] = first_element['video_url_list'][0]
        ...
        # 保存数据到 INI 文件
        with open('data.ini', 'a') as configfile:
